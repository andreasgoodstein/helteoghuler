...
{
    private readonly PackedScene PublicInnkeeperScoreItem = GD.Load<PackedScene>(
        "res://scenes/components/PublicInnkeeperScoreItem.tscn"
    );
    ...
    {
        var publicPlayerCollection =
            GlobalGameState.Get()?.PublicPlayerDict?.Values
            ?? new Dictionary<Guid, PlayerPublic>().Values;
        var publicPlayerList = publicPlayerCollection
            .ToList()
        ...
        foreach (PlayerPublic publicPlayer in publicPlayerList)
        {
            ...
            item.GetNode<Label>("Container/CareerGold").Text =
                publicPlayer.TotalGoldEarned.ToString();
